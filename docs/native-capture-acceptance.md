...
| --- | --- | --- |
| Shared capture contract | `app/platforms/capture/base.py` and `models.py` | `test_capture_backends.py`, `tests/benchmark/test_capture.py` |
...
| Privacy | No pixel data in diagnostics, benchmark summaries, or stability output | diagnostics and `tests/benchmark/` |
...
```bash
python -m compileall -q app developer scripts tests main.py developer_main.py
...
For long-run checks, follow `docs/capture-soak-testing.md` and run at least the
one-hour Auto, native-only, and MSS-only matrix. The eight-hour Auto run is
recommended for a release candidate.
...
one-hour stability runs are recorded without stale frames, unbounded memory/resource
